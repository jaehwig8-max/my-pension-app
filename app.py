...
# 캐시 시간을 1시간(3600초)으로 늘려 요청 횟수를 최소화합니다.
...
def get_market_data():
    ...
    try:
        for name, ticker_symbol in tickers.items():
            ...
            if hist.empty:
                raise ValueError(f"{name} 데이터를 가져올 수 없습니다.")
...
# 에러가 발생했을 경우 안내 메시지 출력
if error_msg:
    st.error(f"⚠️ 야후 파이낸스 서버 연결이 원활하지 않습니다. (Rate Limit)")
    st.info("잠시 후 페이지를 새로고침(F5) 해주세요. 캐시 데이터가 있으면 곧 정상화됩니다.")
    st.stop()
...
st.divider()
st.caption("Rate Limit 완화를 위해 데이터는 1시간 단위로 업데이트됩니다.")
